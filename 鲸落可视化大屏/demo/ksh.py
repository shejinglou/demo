...
def yqbt(): # 疫情饼图
    ...
    df = df.astype({'currentconfirmedcount': 'int64', 'confirmedcount': 'int64', 'curedcount': 'int64',
                    'deadcount':'int64'})
    ...
    df['provincename'] = df['provincename'].str.\
        strip('省').str.strip('市').str.strip('壮族自治区').\
        str.strip('自治区').str.strip('回族自治区').\
        str.strip(
    ...
    for i in range(len(x)):
        c = (
                ...
                .set_global_opts(title_opts=opts.TitleOpts(title='中国疫情情况',
                                                       pos_left='75%', pos_top='5%',
                                                       title_textstyle_opts=opts.TextStyleOpts(color='#ffffff')),
...
def bg():
    ...
    c = (
            ...
            .add_xaxis(address)
            .add_yaxis('', xyqz)
            .set_series_opts(markline_opts=opts.MarkLineOpts(data=[opts.MarkLineItem(type_="max",name="max"),
                                                                   opts.MarkLineItem(type_="min",name="min")]),
                                    markpoint_opts=opts.MarkPointOpts(data=[opts.MarkPointItem(type_="max",
                                                                                               name="max"),
                                                                            opts.MarkPointItem(type_="min",
                                                                                               name="min")]))
            .set_global_opts(title_opts=opts.TitleOpts(title='近期31省区市本土现有病例',
                                                       title_textstyle_opts=opts.TextStyleOpts(color='#ffffff'),
                ...
                datazoom_opts=opts.DataZoomOpts(is_show=False, type_='inside', range_start=30),
                 legend_opts=opts.LegendOpts(pos_right='5%', pos_top='5%',
                                             textstyle_opts=opts.TextStyleOpts(color='#ffffff')),
                 yaxis_opts=opts.AxisOpts(name="数量",  axislabel_opts=opts.LabelOpts(rotate=90,
                                                                                    color='#ffffff'),
...
def gfx(t):
    ...
    c = (
        Tree()
        .add("", [j], collapse_interval=20, is_roam=True, layout="radial",
             initial_tree_depth=y,  label_opts=opts.LabelOpts(color='#ffffff', font_size=9))
        .set_global_opts(title_opts=opts.TitleOpts(title=f"{t}地区",
                                                   title_textstyle_opts=opts.TextStyleOpts(color=x, font_size=16),))
    )
    return c.dump_options_with_quotes()