...
con = create_engine("mysql+mysqlconnector://root:root@localhost:3306/yq1")
# 历史数据
def lishishuju():
    df = pd.read_csv('D:\桌面\COVID-19-KSH-main\demo\csv\丁香园国内疫情.csv', engine='c') # 读取数据
    ...
    df.to_csv('D:\桌面\COVID-19-KSH-main\demo\csv\国内疫情数据.csv', index=False, encoding='utf-8-sig')
...
def yqday():
    ...
    with open('D:\桌面\COVID-19-KSH-main\demo\data\中国疫情.json', 'w', encoding='utf-8') as f:
        f.write(json.dumps(data, ensure_ascii=False, indent=4))

    pd.DataFrame(y).to_csv('D:\桌面\COVID-19-KSH-main\demo\csv\近31省市区现有本土病例.csv', index=False, encoding='utf-8', header=x)
    # 使用create_engine + pandas 快捷保存数据库
...
def yqveryday():
    ...
    # 使用pandas快捷保存csv
    pd.DataFrame(data).to_csv('D:\桌面\COVID-19-KSH-main\demo\csv\丁香园国内每日疫情情况.csv', encoding='utf-8', index=False)
...
def ssrd():
    ...
    data = json.loads(requests.get(url=url, headers=headers).content.decode().split('(')[1][:-1])['Result'][0]['DisplayData']['result'][
        'items']
    with open('D:\桌面\COVID-19-KSH-main\demo\data\实时热点.json', 'w', encoding='utf-8') as f:
        f.write(json.dumps(data, ensure_ascii=False, indent=4))
...
def parse():
    ...
    pd.DataFrame(data1).to_csv('D:\桌面\COVID-19-KSH-main\demo\csv\丁香园国内疫情.csv', encoding='utf-8', index=False)
...
def fxdq():
    ...
    for i in range(len(resdata)):
        ...
        with open(
                # f'demo/data/{fxlevel[tt]}.json'
                f'D:\桌面\COVID-19-KSH-main\demo\data\{fxlevel[tt]}.json', 'w', encoding='utf-8') as f:
            f.write(json.dumps(q, ensure_ascii=False, indent=4))
    with open(f'D:\桌面\COVID-19-KSH-main\demo\data\风险地区.json', 'w', encoding='utf-8') as f:
        f.write(json.dumps(data, ensure_ascii=False, indent=4))
...
def Moon_Tow_Near():
    ...
    pd.DataFrame(df).to_csv('D:\桌面\COVID-19-KSH-main\demo\csv\近2个月新增情况.csv', encoding='utf-8', index=False)
    pd.DataFrame(df1).to_csv('D:\桌面\COVID-19-KSH-main\demo\csv\近2个月累计情况.csv', encoding='utf-8', index=False)
